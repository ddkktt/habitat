...
An article qualifies if it describes a problem with public infrastructure or services affecting residents. Qualifying topics: roads and potholes, water supply, water leaks, drainage and sewage, flooding, street lighting, electric power, trash collection, public spaces and green areas, sidewalks, bridges, and public transit conditions.
...
## Task 2: Extract one record per article

For each qualifying article, produce exactly this JSON structure:
...
```json
{
  ...
  "article_date": "",
  "source_outlet": "",
  "author": "the journalist's byline as printed, or null if unsigned",
  "qualifies": "yes | unsure",
  "categories": ["roads | water | drainage | flooding | lighting | power | trash | public_space | transit | other"],
  "status": "new_complaint | ongoing | failed_repair | resolved | unclear",
  "location_certainty": "exact | approximate | none",
  "location_evidence": "exact phrase copied from the article that names the place, or null",
  ...
  "summary": "one sentence, maximum 25 words, in English",
...
1. **No evidence, no location.** You may only fill `street`, `colonia`, or `landmark` if `location_evidence` contains the actual words from the article supporting it. If you cannot quote it, the value is null.
...
Before saving a record, compare it against records from the previous 14 days. Two articles describe the same incident when they match on category AND location AND overlap in time. When you find a match:
